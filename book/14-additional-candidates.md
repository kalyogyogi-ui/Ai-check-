...
- **FN-DSA** provides the smallest lattice-based signatures (~666 B) but requires significant implementation expertise due to Gaussian sampling complexity and side-channel risks. It will be standardized separately from the primary FIPS standards.
- **Classic McEliece** offers the highest cryptanalytic confidence of any KEM (nearly 50 years of study) but with impractical key sizes (261 KB to 1.3 MB). It serves specialized high-assurance applications where keys can be pre-distributed.
- **HQC** provides code-based diversity for key encapsulation, selected for standardization alongside ML-KEM. Its larger sizes (2-6x) are the cost of mathematical diversity.
- **BIKE** offers competitive sizes among code-based KEMs but faces challenges in rigorously proving its decryption failure rate, delaying standardization.
- **Additional signature candidates** (UOV, MAYO, SQISign, CROSS, LESS, MEDS) explore diverse mathematical foundations with dramatically different trade-off profiles. MAYO is particularly promising for its compact combined sizes.
- **Stateful hash-based signatures** (XMSS/LMS) are already standardized and provide the most conservative security assumptions, ideal for controlled-environment signing operations.
- **Algorithm selection** depends on the specific deployment context: general-purpose systems should use ML-DSA, size-constrained systems should consider FN-DSA, and maximum-confidence systems should use SLH-DSA or XMSS/LMS.
- **Organizations should primarily deploy FIPS 203/204/205** for immediate protection while monitoring the standardization of additional algorithms and building flexibility into their cryptographic architectures to incorporate new standards as they emerge.
