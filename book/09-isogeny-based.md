...
- Isogeny-based cryptography uses structure-preserving maps between elliptic curves as its computational foundation, with security based on the difficulty of finding paths in isogeny graphs.
- The supersingular isogeny graph is a Ramanujan expander graph with optimal mixing properties, making path-finding computationally hard (O(p^(1/4)) classical, O(p^(1/6)) quantum).
- SIDH/SIKE was broken in 2022 by Castryck-Decru using the Kani lemma and abelian surface isogenies—a cautionary tale about publishing the action of secret maps on known algebraic structures.
- CSIDH provides a commutative group action enabling Diffie-Hellman-like protocols with 64-byte keys, but faces subexponential quantum attacks via Kuperberg's algorithm for the hidden shift problem.
- SQISign achieves the smallest signatures (177 bytes) of any PQC scheme by exploiting the Deuring correspondence between supersingular curves and quaternion algebras, at the cost of slow signing (~1-5 seconds).
- The SIDH attack does not affect CSIDH (no auxiliary points) or SQISign (different paradigm), demonstrating that the isogeny family is diverse, not monolithic.
- Isogeny-based cryptography excels in compactness but suffers from computational expense, limiting practical applications to scenarios where size is paramount.
- The field lacks cryptanalytic maturity compared to lattices or codes—major surprises may still occur in either direction.
- No isogeny-based algorithm has been standardized by NIST; SQISign remains under evaluation for additional signatures.
- Current recommendation: Do not deploy isogeny-based schemes in production; monitor SQISign's standardization progress and research developments for future adoption when maturity is established.
- The mathematical richness of isogenies enables unique cryptographic primitives (NIKE, compact group actions) unavailable from other PQC families, ensuring continued research relevance regardless of standardization outcomes.
