...
The consequence is clear: implementing post-quantum cryptography securely requires understanding both the mathematical structure of the algorithms and the physical characteristics of the target platform. This chapter provides the technical foundation for secure PQC implementation across the full spectrum of deployment targets.
...
- The implementation gap for PQC is wider than for classical cryptography due to novel operations (NTT, rejection sampling), larger state, and less mature tooling
- Constant-time implementation is mandatory for all PQC algorithms — secret-dependent branches, memory accesses, and variable-time arithmetic must be eliminated
- Side-channel attacks (timing, power/EM, fault, cache) require layered countermeasures including constant-time code, masking, redundant computation, and hardware protections
- Masking the NTT is efficient due to its linearity, but pointwise multiplication requires expensive secure protocols — first-order masking costs 2-4x overhead
- Platform-specific optimization is essential: AVX2 on x86-64, NEON on ARM, streaming computation on Cortex-M4, dedicated engines on FPGA/ASIC
- High-quality randomness (hardware entropy + approved DRBG) is critical — poor randomness can completely compromise PQC key generation and encapsulation
- Testing must encompass correctness (KATs/ACVP), interoperability (cross-library), side-channel resistance (TVLA), and robustness (fuzzing, fault simulation)
- Common pitfalls include non-constant comparisons, compiler-eliminated zeroization, variable-time reduction, rejection state leakage, and insufficient entropy
- Use established, maintained libraries with appropriate side-channel protection for your threat model — avoid implementing PQC from scratch unless you have specialized expertise
