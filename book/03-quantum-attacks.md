...
The quantum threat to cryptography is not speculative or distant — it is a mathematically proven vulnerability with a timeline measured in years, not generations. The key conclusions from this chapter are:

**Shor's Algorithm is Existential for Public-Key Cryptography:** Shor's algorithm reduces factoring and discrete logarithm from sub-exponential/exponential to polynomial time — an unbridgeable gap that no key size increase can address. Every deployed public-key algorithm (RSA, DH, ECDH, ECDSA, EdDSA) is completely and irrecoverably broken by a sufficiently large quantum computer. This is not a matter of increasing key sizes; the mathematical structure that makes these problems hard for classical computers is precisely what makes them easy for quantum computers.

**Grover's Algorithm is Manageable for Symmetric Cryptography:** Grover's quadratic speedup halves the effective security of symmetric algorithms, but this is easily mitigated by doubling key sizes. Practical limitations (sequential nature, circuit depth, parallelism disadvantage) further reduce the real-world threat. AES-256 and SHA-384/SHA-512 provide adequate post-quantum security with minimal performance impact.

**The Boundary Between Quantum-Easy and Quantum-Hard is Well-Characterized:** Quantum computers excel at problems with hidden algebraic structure (periodicity, group structure, abelian symmetry). Problems without such structure — random lattice problems, decoding random codes, inverting random functions — resist quantum speedups beyond Grover's quadratic improvement. This boundary defines the design space for post-quantum cryptography.

**Harvest Now, Decrypt Later Makes the Threat Immediate:** The retroactive nature of the quantum threat to confidentiality means that migration urgency is not determined by when quantum computers arrive, but by how long encrypted data must remain secret. For diplomatic, military, health, and personal data with multi-decade secrecy requirements, the threat is already present.

**Complete Infrastructure Replacement is Necessary:** Unlike the Y2K problem (which required updating date fields), post-quantum migration requires replacing fundamental cryptographic algorithms throughout the entire digital infrastructure — every protocol, every implementation, every device. The scale and complexity of this undertaking demands immediate action despite uncertainty about the exact timeline of quantum computer development.

**Post-Quantum Solutions Exist:** The situation is urgent but not hopeless. Mathematical problems that resist quantum attacks have been identified, standardized, and implemented. The NIST Post-Quantum Cryptography standards (ML-KEM, ML-DSA, SLH-DSA) provide concrete, deployable replacements. The remaining challenge is one of engineering and coordination, not fundamental science.
