...
Lattice-based cryptography stands as the most versatile and extensively studied family of post-quantum cryptographic constructions. Its prominence derives from three remarkable properties: strong theoretical foundations rooted in the worst-case hardness of well-studied geometric problems, practical efficiency that rivals and sometimes surpasses classical cryptographic schemes, and extraordinary versatility that enables not only basic primitives like encryption and signatures but also advanced constructions including fully homomorphic encryption and attribute-based encryption. This chapter develops the mathematical foundations of lattice-based cryptography from first principles, traces the evolution from theoretical problems to practical schemes, and examines the concrete security considerations that govern real-world parameter selection.
...
- **Lattices are discrete geometric objects** in high-dimensional space. The hardness of finding short vectors grows exponentially with dimension, providing the foundation for post-quantum security.

- **LWE is the central hardness assumption.** Regev's quantum worst-case to average-case reduction ensures that breaking LWE-based schemes is at least as hard as solving lattice problems on arbitrary lattices in the worst case.

- **Module-LWE balances security and efficiency.** By operating over vectors of ring elements, Module-LWE achieves practical key sizes while maintaining security confidence beyond pure Ring-LWE, and this is the choice underpinning NIST's standardized algorithms.

- **The Fujisaki-Okamoto transform** elevates passively secure encryption into actively secure KEMs through deterministic re-encryption and implicit rejection, providing the IND-CCA2 security required by real-world protocols.

- **Lattice-based signatures follow two paradigms:** the GPV hash-and-sign approach (compact signatures but complex sampling, as in FALCON) and Fiat-Shamir with aborts (larger signatures but simpler implementation, as in ML-DSA).

- **Lattice reduction algorithms determine concrete security.** BKZ with sieving subroutines represents the state of the art in lattice attacks. The Core-SVP methodology translates algorithmic costs into bit-security estimates for parameter selection.

- **Quantum computers offer only modest speedups** against lattice problems. The reduction from exponential classical time 2^(0.292n) to quantum time 2^(0.265n) is far less dramatic than the exponential-to-polynomial speedup Shor's algorithm provides against factoring.

- **Lattices uniquely support advanced primitives.** No other post-quantum family approaches lattices in versatility: fully homomorphic encryption, attribute-based encryption, and efficient zero-knowledge proofs all depend critically on lattice-based assumptions.

- **Concrete parameters are well-understood.** ML-KEM's parameters (n=256, q=3329, k=2/3/4) achieve NIST security levels 1/3/5 with comfortable margins against known attacks, small key and ciphertext sizes, and negligible decryption failure probabilities.
