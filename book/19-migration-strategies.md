...
Migrating to post-quantum cryptography is one of the largest coordinated technology transitions in history. Unlike the Y2K remediation, which had a fixed and immutable deadline, the PQC migration faces an uncertain timeline driven by quantum computing progress — yet the urgency is no less real. Organizations that store long-lived sensitive data face risk today, because adversaries can record encrypted communications now and decrypt them later once quantum computers reach sufficient capability.

This chapter provides a systematic framework for planning and executing the migration to post-quantum cryptography, from initial assessment through full deployment and beyond.
...
- Cryptographic agility is both the enabler of the current PQC migration and the architectural principle that prevents future migrations from being equally disruptive. Invest in agility as a first-class architectural property, not as a one-time project goal.

- The migration framework follows a clear sequence: Inventory → Risk Assessment → Architecture Planning → Pilot Deployment → Broad Rollout → Classical Deprecation. Skipping steps creates risk; each phase builds on the previous and provides inputs to the next.

- Prioritization must be risk-driven: data subject to HNDL attacks with long confidentiality lifetimes demands immediate action on PQC key exchange, regardless of the uncertainty about CRQC timelines. Mosca's inequality provides the rational framework for urgency assessment.

- Hybrid deployment (classical + PQC) is the universally recommended transition pattern for the current period. It maintains classical security as a floor while adding quantum resistance. Plan for eventual transition from hybrid to PQC-only, but do not rush the classical deprecation.

- Dependencies — cryptographic libraries, HSMs, certificate authorities, cloud providers, and partner organizations — frequently determine the critical path for migration. Identify blocking dependencies early and begin resolution in parallel with other migration activities.

- Testing must be comprehensive, covering interoperability, performance, security, and failure modes. PQC changes system behavior in ways that are not always obvious — timing characteristics, message sizes, error handling, and resource consumption all shift. Production issues discovered late are costly.

- Government mandates (CNSA 2.0, EU NIS2, FIPS requirements) provide concrete timelines that make migration non-optional for many organizations. Even organizations not directly subject to these mandates will be affected through supply chain requirements and industry expectations.

- Organizational readiness requires executive sponsorship, dedicated staffing, adequate budget, and cross-functional coordination. PQC migration is not a purely technical project — it requires alignment across security, engineering, compliance, procurement, and business leadership.

- Measure progress continuously. Concrete metrics on PQC coverage, risk reduction, and operational impact enable evidence-based decisions and maintain organizational momentum through a multi-year effort.

- Start now. Even if the CRQC timeline is uncertain, the migration itself takes years. The intersection of data lifetime, migration duration, and threat timeline (Mosca's inequality) means that delay creates unrecoverable risk for long-lived data. Organizations that begin today will be prepared regardless of when quantum computers arrive.

- Build for the future: the goal is not just to deploy today's PQC algorithms, but to build systems capable of adapting to whatever cryptographic challenges emerge next. The PQC migration should be the last painful cryptographic transition your organization endures — because it establishes the agility to handle future transitions gracefully.
