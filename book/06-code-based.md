...
This chapter provides a comprehensive treatment of code-based cryptographic systems. We begin with the mathematical foundations of error-correcting codes, then examine the McEliece cryptosystem and its variants in detail, analyze the computational hardness assumptions underlying these schemes, survey modern compact-key constructions that have advanced to NIST standardization, and discuss the significant open challenges that remain in this field.
...
Code-based cryptography occupies a unique position in the post-quantum landscape. Its principal advantages and challenges can be summarized as follows:

**Strengths:**
- The longest security track record of any post-quantum family: the McEliece system has withstood nearly 50 years of cryptanalysis without requiring parameter increases.
- Security is based on the syndrome decoding problem, which is NP-hard and has no known polynomial-time quantum algorithm. The best quantum attacks provide only a quadratic speedup over classical algorithms.
- The mathematical foundations are well-understood: coding theory has been studied extensively since the 1950s, and the behavior of random linear codes is well-characterized by the Gilbert-Varshamov bound and related results.
- Fast operations: code-based systems typically require only XOR and bitwise operations, making them efficient on a wide range of platforms.
- HQC has been selected by NIST for standardization, providing algorithm diversity alongside lattice-based ML-KEM.

**Challenges:**
- Large public keys remain the primary practical limitation, particularly for Classic McEliece (hundreds of kilobytes to over a megabyte).
- Code-based digital signatures remain an open research problem, with no scheme achieving the combination of small size, fast operation, and well-understood security enjoyed by lattice-based or hash-based alternatives.
- Structured variants (QC codes) introduce additional algebraic assumptions whose long-term security is less certain than that of random Goppa codes.
- Constant-time implementation of decoders (particularly iterative decoders for MDPC codes) requires significant engineering effort.
- The gap between worst-case NP-hardness and average-case hardness for specific parameter choices is bridged primarily by empirical evidence rather than formal reductions.

**Looking Forward:**
The field continues to advance on multiple fronts. Research into more compact code-based signatures (LESS, MEDS, Wave) may eventually yield practical schemes. Better understanding of quasi-cyclic code security strengthens confidence in BIKE and HQC. New metrics (rank metric, Lee metric) offer potential for more compact schemes once their security is better understood. And the ongoing development of quantum computing hardware provides growing motivation for the deployment of code-based systems whose security does not depend on the intractability of structured algebraic problems vulnerable to quantum algorithms.

The selection of HQC by NIST for standardization marks a significant milestone: for the first time, a code-based KEM will be available as a widely deployed standard, ensuring that the post-quantum cryptographic ecosystem does not rely exclusively on lattice-based hardness assumptions. Combined with Classic McEliece's unparalleled security confidence for high-assurance applications, code-based cryptography will play a central role in the post-quantum transition.
