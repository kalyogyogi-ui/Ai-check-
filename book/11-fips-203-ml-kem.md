...
- ML-KEM (FIPS 203) is the primary post-quantum key encapsulation standard, providing IND-CCA2 security based on the Module-LWE hardness assumption
- The scheme operates in the polynomial ring Z_3329[X]/(X^256 + 1) with module rank k ∈ {2, 3, 4} providing three security levels targeting NIST Levels 1, 3, and 5
- The NTT enables O(n log n) polynomial multiplication and dominates computational cost; the choice of q = 3329 allows complete NTT decomposition for maximum efficiency
- Three algorithms—KeyGen, Encaps, Decaps—implement key pair generation, shared secret creation, and shared secret recovery with re-encryption verification for CCA2 security
- The Fujisaki-Okamoto transform with implicit rejection provides CCA2 security from a CPA-secure core, preventing chosen-ciphertext and oracle attacks
- Centered Binomial Distribution noise sampling enables trivially constant-time implementation, unlike Gaussian sampling which introduces timing side-channels
- ML-KEM-768 is the recommended default: ~2,272 bytes total bandwidth, ~100 μs operations on modern x86-64, security well above NIST Level 3
- All known attacks reduce to lattice reduction (BKZ + sieving), with best classical costs of ~2^196 and best quantum costs of ~2^178 for ML-KEM-768
- Deployment should use hybrid mode (ML-KEM + X25519) during the transition period, providing security against both classical and quantum adversaries regardless of which assumption holds
- Implementations must be constant-time (no secret-dependent branches or memory accesses), use approved random number generators, and validate public keys before use
