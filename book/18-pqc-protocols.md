...
The deployment of post-quantum cryptography does not happen in a vacuum. Cryptographic algorithms exist within complex protocol ecosystems that have been refined over decades. Integrating PQC into these protocols requires understanding the constraints each protocol imposes — message size limits, round-trip expectations, backward compatibility requirements, and performance budgets. This chapter examines how PQC integrates into the major network protocols and infrastructure systems that underpin modern communications.
...
- TLS 1.3 hybrid key exchange is already deployed at scale (Chrome, Cloudflare, AWS) with measured latency impact of less than 1 ms on typical connections. Certificate compression and caching mitigate the authentication size overhead.

- PKI migration requires a carefully phased approach through hybrid intermediates, cross-signed PQC roots, and eventual PQC-only operation. The certificate ecosystem's hierarchical nature means changes propagate slowly through trust stores and client deployments.

- SSH has the most straightforward migration path among major protocols, with hybrid key exchange already available in OpenSSH and minimal protocol changes required. The key management model (TOFU or certificates) accommodates larger keys naturally.

- VPN protocols (IPsec/IKEv2, WireGuard, OpenVPN) each have viable PQC integration paths, though fragmentation concerns for IPsec and WireGuard's compact design philosophy require careful engineering solutions.

- Email security (S/MIME, OpenPGP) faces compounding size challenges from encryption, signatures, and certificate distribution, but the store-and-forward nature means latency is less critical than in interactive protocols.

- DNSSEC faces the most severe challenges due to fundamental UDP size constraints and per-query overhead. Alternative algorithms (FN-DSA) and protocol redesigns may be necessary before DNSSEC can fully adopt PQC.

- Code signing and secure boot must balance PQC protection for long-lived firmware with NVRAM constraints and verification performance. Stateful hash-based signatures (XMSS/LMS) offer conservative options for controlled signing environments.

- Blockchain systems must solve the fundamental tension between PQC's larger keys/signatures and blockchain's per-transaction costs, while managing migration across decentralized ecosystems.

- IoT and constrained protocols require per-application analysis of resource budgets, with solutions ranging from pre-provisioned keys to proxy-assisted architectures that offload PQC computation.

- The overall transition is technically feasible for all examined protocols. The challenge is engineering — managing interoperability during transition periods, optimizing implementations for real-world constraints, and coordinating deployment across diverse ecosystems.
