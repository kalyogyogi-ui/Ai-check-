...
- Multivariate cryptography is based on the NP-hard MQ problem, which has no known efficient quantum algorithm.
- The trapdoor paradigm disguises an invertible central map with secret affine transformations, producing a public key that appears to be a random polynomial system.
- UOV is the primary surviving multivariate signature scheme, with 25+ years of cryptanalytic confidence and extremely small signatures (112 bytes at Level I).
- Rainbow's break in 2022 by Beullens demonstrated that multi-layer multivariate constructions create exploitable inter-layer algebraic relationships not present in single-layer designs.
- MAYO represents the modern evolution of multivariate signatures, achieving compact keys through the "whipping" technique while maintaining the UOV security foundation.
- Gröbner basis algorithms (F4, F5) are the primary attack tools; their complexity is governed by the degree of regularity, which grows super-polynomially for well-parameterized schemes.
- Quantum attacks on MQ provide only modest polynomial speedups, making multivariate schemes naturally quantum-resistant without requiring large parameter increases.
- The main practical weakness is large public keys (tens of kilobytes for UOV), though MAYO and similar techniques are narrowing this gap.
- Multivariate signatures excel in applications prioritizing small signatures, fast signing/verification, and quantum resistance, while being less suited for memory-constrained devices or frequent key distribution.
