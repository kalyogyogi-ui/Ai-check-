...
- PQC standardization is a beginning, not an end — the FIPS 203/204/205 standards launch a multi-decade research and deployment journey
- Advanced cryptographic primitives (FHE, ZK proofs, MPC, ABE) built from lattice assumptions are progressing toward practicality, with FHE leading and others following on 5-10 year timelines
- Signature aggregation remains the single most impactful open problem for PQC scalability — its solution would transform blockchain, IoT, and certificate infrastructure
- IoT and embedded PQC requires fundamental research beyond simple optimization, potentially including new lightweight assumptions, split architectures, and hardware co-design
- Cryptanalysis must continue vigilantly on deployed standards — the community's responsibility does not end with standardization. Continuous monitoring, diverse algorithm portfolios, and cryptographic agility provide resilience
- QKD and PQC serve complementary roles rather than competing — PQC for general-purpose Internet security, QKD for ultra-high-security point-to-point links, with decades before quantum Internet matures
- Formal verification of PQC implementations is increasingly important as deployment expands, with significant progress from tools like Jasmin, EasyCrypt, and CryptoLine, but scalability to library-level verification remains open
- The transition spans decades through deployment (2025-2030), maturation (2030-2040), and quantum arrival (2040+) — we are in the early deployment phase where foundational decisions shape long-term outcomes
- The field offers abundant high-impact research problems spanning theory (reductions, cryptanalysis), engineering (implementation, optimization), and applications (IoT, blockchain, privacy)
- Organizations should deploy current standards confidently while monitoring research advances and maintaining cryptographic agility for future algorithm updates
