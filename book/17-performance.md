...
- ML-KEM is computationally competitive with or faster than classical ECDH at equivalent security levels — the speed concern for PQC key exchange is largely resolved
- ML-DSA signing is 10-20x slower than ECDSA, but verification is only 2-3x slower — acceptable for virtually all applications where verification dominates (certificate chain validation, signature checking)
- SLH-DSA is 100-1000x slower than ECDSA for signing — suitable only for infrequent signing (CA operations, firmware updates, root certificates) but provides conservative hash-based security
- The primary PQC overhead is bandwidth (10-50x larger keys and signatures), not computation — system architects should focus on communication optimization
- Hybrid schemes (classical + PQC) roughly double both computation and bandwidth compared to PQC alone, providing defense-in-depth at manageable cost
- TLS handshake with hybrid PQC adds 2-5 ms latency on broadband and potentially one additional RTT on bandwidth-constrained links where the handshake exceeds the initial congestion window
- Constrained devices (Cortex-M4) can run ML-KEM in ~1.5 ms and ML-DSA verification in ~10 ms, with memory (6-48 KB stack) as the binding constraint rather than computation speed
- Energy analysis shows ML-KEM is actually more energy-efficient than classical ECDH, while ML-DSA signing costs 7-11x more energy — communication energy dominates for wireless devices
- Bandwidth-constrained environments (LoRaWAN, satellite, BLE) require careful protocol design: pre-shared keys, asymmetric authentication frequency, or compact signature schemes
- NTT optimization (merged layers, Montgomery multiplication, AVX2/NEON vectorization) provides the largest single performance improvement for lattice-based schemes
- Hash function parallelism (4-way SHAKE with SIMD) significantly accelerates SLH-DSA and the hash-heavy portions of ML-KEM/ML-DSA
- Hardware acceleration (dedicated NTT instructions, PQC coprocessors) expected by 2027-2028 will provide an additional 2-5x improvement over current optimized software
- Benchmark methodology matters: report median and percentiles (not just mean), stabilize CPU frequency, account for ML-DSA signing variance, and document the full measurement environment
